...
**Mentorship Tracker — Department picker expansion + coming-soon screens (Scott, branch `claude/serene-euler-vF8et`, HEAD `55f1811`, 2026-05-26).** Added Urgent Care as 4th selectable department option. All three non-Primary-Care departments (Pediatrics, Specialty, Urgent Care) are now clickable and route to department-specific coming-soon screens instead of a generic message. Each screen has: dark-gradient header with dept icon + "Coming Soon" badge, named Medical Director strip (Pediatrics: Dr. Sarah Chen #059669; Specialty: Dr. Michael Torres #7c3aed; Urgent Care: Dr. Rachel Kim #dc2626), description paragraph, 4 feature-preview tiles. Back to Department Selection returns to picker. Primary Care flow unchanged. Build clean, smoke test 11/11. **Branch HEAD 55f1811, PR #10 open, awaiting Noah squash-merge.** **Resume recipe after `/clear`:** branch `claude/serene-euler-vF8et` at `55f1811`; PR #10 open; only action needed is Noah's squash-merge approval.

**Mentorship Tracker — Curriculum additions + My Panel chip (Scott, branch `claude/serene-euler-vF8et`, HEAD `4164ecd`, 2026-05-26).** (a) My Panel mentee cards now show "✓ [phase] complete" chip when all current-phase items are done (was blank). (b) Controlled substances thread: Mentor Month 1 — ask how they're handling inherited opioid/benzo/stimulant patients + share your approach; Mentor Month 3 — debrief cases where they felt stuck; Medical Director Month 6 — check in on progress. (c) Clinical modules thread: Medical Director Week 1 — intro to the Meridian modules + walk-through (md-63); Medical Director Week 1 — module review items for opiates/benzos/ADHD (md-64, md-65, md-66); Mentor Month 1 — ask if they've opened the modules; Mentor Month 3 — debrief which modules have been opened/useful; Medical Director Month 6 — check in on which modules relied on (md-68). Master-checklist now has 68 items (up from 62). **Not yet merged to main — PR #10 pending.**

**Mentorship Tracker — Needs Attention banner + UI refinements (Scott, branch `claude/serene-euler-vF8et`, HEAD `b162eba`, 2026-05-25).** Session 2026-05-25 added: (a) warm-amber "Needs Attention" banner on director roster — auto-scans all providers via `getStatus()`, surfaces LATE/DUE rows with phase label, day count, score trend direction (score↑/↓ from last two QP averages), and mentor surname; each row navigates to that provider's Mentor tab on click; hides automatically when all ok; (b) dept picker as first screen (Primary Care → main app, Pediatrics/Specialty → "coming soon"); (c) two-column login/dept layout (`BrandPanel` 42% + `CardPanel` 58%) using `gradBg` fixed-fullscreen overlay; (d) "Optum NY/NJ" branding throughout (replaced Crystal Run Healthcare); (e) phase buttons grouped into labeled rows (Wk 0–Mo 2, Wks 5–8, Mo 3–12, Mo 15–24) using `getGroupLabel()`; (f) same grouping applied to profile phase chip strip via `chipGroupLabel()`; (g) "Advance →" promoted to real teal CTA button; (h) "Dir. Curriculum" sidebar bar label fix (was "Medical Director" truncated); (i) 4 new seed providers (Hassan, Park, Okafor, Singh) + APC mentor M. Torres NP; (j) "MD" label fix everywhere it meant Medical Director (not physician credential). **Branch HEAD b162eba, PR #10 open, awaiting Noah squash-merge.** **Resume recipe after `/clear`:** branch `claude/serene-euler-vF8et` at `b162eba`; PR #10 open; only action needed is Noah's squash-merge approval. Pending further MD dashboard improvements: mentor health summary cards, "due this month" timeline, cohort swim-lane, score sparklines in roster (all discussed, not yet implemented).

**Mentorship Tracker — localStorage persistence + data management (Scott, branch `claude/serene-euler-vF8et`, HEAD `a01ef2e`, 2026-05-23).** Adds full session persistence and director data-management tools. Key changes: (a) autosave to `localStorage` key `meridian-mt.v1` on every change to `provs/checks/qa/notes` via `useEffect`; (b) login screen detects prior sessions (`loadMT() !== null`) and shows "Saved session found" banner + "Start Fresh…" button with two sub-options: "Clear checks/scores/notes (keep providers)" or "Reset everything (restore seed defaults)"; (c) director top bar gains: ⬇ JSON export (downloads all session data as dated JSON), 📄 DOCX export (async dynamic import of `docx` library, one document with one section per provider: MD curriculum, Mentor Physician track, APC track if applicable, QA scores, notes), ↺ Reset (opens same two-option reset modal); (d) sidebar: each provider card has an `×` remove button with confirm modal; "+ Add Provider" button at bottom opens modal (name, role MD/DO/NP/PA, mentor dropdown, starting phase, days); (e) provider profile card: "Days" label becomes click-to-edit inline input (Save/✕/Enter/Escape); "Advance → [next phase]" button appears when at non-final phase. `SEED_PROVS` constant preserves original 5 providers; module-level `let PROVS` is synced to `provs` state on every render so all existing helper functions (mentorPhysPct, getStatus, detectPatterns, etc.) see current state without refactoring. **Branch also includes all prior commits:** full Mentor Curriculum, APC track, MD touchpoint questions, nav-history bug fix, etc. **Not yet merged to main** — PR #10 pending Noah review/merge. **Resume recipe after `/clear`:** branch `claude/serene-euler-vF8et` at `a01ef2e`; PR #10 open; only action needed is Noah's squash-merge approval.

**Mentorship Tracker — Full Mentor Curriculum + APC Track (Scott, branch `claude/serene-euler-vF8et`, HEAD `ea466f0`, 2026-05-23).** Replaced the 14-phase stub `MP` with Noah's complete Mentor Curriculum: 17-phase Physician track (Week 0 Pre-Start → Month 12 Year One Completion, 128 items) + 4-phase APC track (Month 15–24, 29 items) for a total of 157 items. Key changes: (a) `MP_PHYS` + `MP_APC` + `MP = [...MP_PHYS, ...MP_APC]`; (b) `isAPC()` helper checks role NP/PA; (c) `mentorPhysPct()` + `mentorApcPct()` split helpers; (d) "Mentor Curriculum" metric tile split into "Mentor: Physician" and "Mentor: APC" (APC tile only shown when viewing an NP/PA provider); (e) both tracks accessible in "View All Curriculum" modal as separate color-coded sections; (f) Phase IDs updated: w4→m1, w8→m2, q3→m9, q4→m12; (g) Dr. Martinez (NP) demo provider at Month 18. **Latest system eval PASSED (2026-05-23, Playwright):** all 5 providers render; Score Trends (15 SVGs, all 3 chart headers); A. Martinez NP 6-tile metric grid (incl. "Mentor: APC"); APC phase buttons M15/M18/M21/M24; no console errors. **Bug fixed 2026-05-23 (ea466f0):** `navigate()` was pushing `{uid:null}` to navHistory on login → the ‹back pill (fixed, top:62, zIndex:1000) overlapped the "Provider Roster" tab (y=66) → clicking that tab triggered `goBack()` which restored uid:null and logged out the director. Fix: skip navHistory push when `uid === null` (i.e. during login transition). **Branch also includes:** `4a84029` MD Touchpoint question replacement; `92ef38e` inter-instance CLAUDE.md note; `c80a4dd` APC provider name fix (NPs are not doctors). **Not yet merged to main** — PR #10 pending Noah review/merge. **Resume recipe after `/clear`:** branch `claude/serene-euler-vF8et` at `ea466f0`; PR #10 open; only action needed is Noah's squash-merge approval.

**Briefing app — PR #8 open 2026-05-22 (Noah, branch `noah/briefing-launcher-app`, HEAD `15aa4dd8`).** New fourth launcher app — port of `~/Downloads/meridian-briefing.html` (Crystal Run Provider Briefing POC) into Preact/TSX. Files: `src/apps/briefing/BriefingApp.tsx` + `src/apps/briefing/briefing.css` (all selectors scoped under `.briefing-app` so the briefing's `.bubble` rule doesn't collide with meridian-os's first-class bubble primitives) + `src/shell/BriefingShell.tsx`. Six-touchpoint launcher add: type extension, route branch, button, icon-variant gradient (navy `#1a2744` → teal `#00788a`), shell, app. Build clean (`npm run build` succeeds, `tsc -b` clean). **Not yet visually tested in browser** — that's the user's first follow-up. PR URL: https://github.com/noahschmuckler/meridian-os/pull/8. Preview URL once Cloudflare build lands: `15aa4dd8.meridian-os.pages.dev`. **Resume recipe after `/clear`:** read this STATE.md entry and CLAUDE.md's 2026-05-22 inter-instance message → check PR #8 status (`gh pr view 8`) → if visual issues found on preview, branch is `noah/briefing-launcher-app`, key files are the BriefingApp.tsx + briefing.css pair; if everything looks good, merge via squash, then bump this STATE.md entry into a Shipped milestone and update the "latest commit on main" pointer in the entry below.
