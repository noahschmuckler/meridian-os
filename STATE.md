...
**Interface-simulacrum PPTX→PDF export — v1 (lipid pilot) BUILT 2026-08-12 (Noah/Fable), **PR #22** open from `noah/interface-pptx-v1`, awaiting Noah's PowerPoint→PDF smoke test.** New export format: a `pptxgenjs`-generated deck whose slides reproduce the Meridian module interface with internal slide-to-slide hyperlinks; PowerPoint "Save as PDF" retains the jumps, yielding an offline single-file interactive PDF for enterprise distribution (meridian-os can't be hosted on enterprise hardware). **Spec + design rationale + implementation notes: [`docs/interface-pptx-export-spec.md`](docs/interface-pptx-export-spec.md)** (spec §1–§13 written earlier on 2026-08-12; §14 implementation notes added at build time). **What shipped:** `src/lib/generateInterfacePptx.ts` (state enumeration → font-scale/pagination → two-pass emit with all slide numbers assigned before any hyperlink resolves), `src/lib/htmlBlocks.ts` (shared `htmlToBlocks` extracted from `generateDocx.ts`; adds per-cell table data + DOM-free scanner fallback so the generator also runs under Node), **"⧉ .pptx"** button in `clinical-module-checklist` chrome (downloads `<module_id>-interactive.pptx`). **Lipid deck verified:** 62 primary states exactly as spec §5.3 predicted (16 faq + 29 sub + 7 pill + home + sp:home + 8 sp), 73 physical slides (11 tall states each spill onto one linked "(cont.)" slide at the ~9pt floor scale); all 2,536 slide-jump links verified against OOXML rels (0 dangling); Node and headless-Chrome builds produce identical output; `tsc -b` + `npm run build` clean. **Key gotcha found (recorded in spec §14):** pptxgenjs silently drops the hyperlink *relationship* when `hyperlink` sits at box level on an array-of-runs `addText` (dead `rIdundefined` links) — must attach the hyperlink to every run. **Both test artifacts handed to Noah in-chat:** `lipid-management-interactive.pptx` (73 slides) + `link-smoke-test.pptx` (3 slides, spec §10). **UPDATE 2026-08-18 — §10 smoke test PASSED (Noah: "worked perfectly" for lipid via PowerPoint→PDF). v1.1 decks generated + delivered in-chat the same day:** adhd (45 primary states / 58 slides), opiates (48/81), benzos (48/72) — all state counts match the spec §5.3 formula, all SmartPhrase pills resolve, all `faq_ref`s resolve, 0 dangling links (7,500 jump actions verified across the three decks), and the `dp:` consult-decision-point states rendered with **zero code changes** — v1.1 was a pure data run via the Node path (`rolldown` bundle + `buildInterfacePptx`, recipe in this bullet's history). **Remaining:** (1) merge PR #22; (2) note the generator supports schema-1.3.0 modules only — ckd/anemia/abd-pain (1.0.x legacy `items[]`) would render an empty detail panel; guard or extend if they ever need this export before their Simplification Pass. **Resume recipe after `/clear`:** read spec §14 + this bullet; branch `noah/interface-pptx-v1`; if PDF links ever break, the encoding layer is `linkRuns()` + `hyperlink:{slide:N}` in `generateInterfacePptx.ts`; to regenerate any deck headlessly: `rolldown src/lib/generateInterfacePptx.ts --format cjs --external pptxgenjs --platform node`, then call `buildInterfacePptx(mod)` + `pres.writeFile` under `NODE_PATH=<repo>/node_modules`.

## Prior current state (last updated 2026-05-30)

_(Note: the 2026-06-04 housekeeping bullet — CLAUDE.md slim PR #19 / Scott's PR #20 / inter-instance PR #21 — lives on the still-open `noah/briefing-sharepoint-port` branch (PR #18) and will land with it.)_
