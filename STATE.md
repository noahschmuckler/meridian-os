...
**Excel workbook export — the Onboarding Tracker moves into the Optum tenant — in flight 2026-08-16 (Scott, branch `claude/zen-knuth-oejubs`).** Optum will not permit hosting anything outside OneDrive, so the Onboarding Tracker program had to leave `meridian-os.pages.dev` and become an Office artifact. New directory `src/apps/mentorship-tracker/excel-export/` generates **four regional `.xlsx` workbooks** (one per Care Delivery Organization) from the curriculum data already in the repo — the repo stays the single source of truth, the workbook is the deliverable.

- **Pipeline:** `extract-data.mjs` (Node) slices the data literals — `MP_PHYS`, `MP_APC`, `OP`, `QP`, `SEED_PROVS`, `USERS`, `PROD_METRICS`, and the `scores` / `omScores` / `ciiScores` seed maps — straight out of `MentorshipTrackerApp.tsx` + `ProductivityPanel.tsx`, plus `mentorship-master-checklist.json`, into `curriculum-data.json`; then `build_workbooks.py` (openpyxl) emits `out/Onboarding-Tracker-<Region>.xlsx`. Extraction throws if a constant is renamed or stops being a literal rather than emitting stale data. `out/` is gitignored; `curriculum-data.json` is committed so the Python step runs without Node.
- **Regions (the CDO model, now enforced by OneDrive sharing instead of an honor-system tile pick):** Hudson Valley East / CareMount / Dr. Scott Freiberg (accent `028090`, ships **with** the 9-provider demo cohort + seeded checks/scores); Hudson Valley West / Crystal Run / Dr. Noah Schmuckler (`7C3AED`); New Jersey / Riverside / Dr. Raj Brahmbhatt (`C2410C`); Long Island / ProHealth / Dr. Pauline Mizrachi (`059669`). The three non-seeded workbooks ship one EXAMPLE roster row showing the expected format. Each Primary Care Medical Director gets their region's file; Scott + Noah are the two program administrators with all four.
- **11 sheets:** `Home` (navy canvas, gridlines off, 8 Mondrian-colored hyperlink tiles each with a live stat formula underneath) · `Roster` (12 provider slots; days-in-practice, expected phase, and On Track/Due/Overdue all computed from start date via the hidden `Lists` phase/day table) · `Dashboard` (5 KPI tiles + per-provider row: completion by track, review + questionnaire averages, C.I.I., metric averages) · `Medical Director Curriculum` (all 68 master-checklist items) · `Mentor - Physician Track` (W0→M12) · `Mentor - APC Track` (M15→M24) · `Office Manager Reviews` (6 checkpoints) · `Provider Questionnaires` (13 checkpoints incl. C.I.I. items) · `Provider Metrics` · `Data Dictionary` · `Lists` (hidden).
- **Provider Metrics is the IT landing zone** — a real Excel Table named `ProviderMetrics` (week ending · provider ID join key · name · the 5 `PROD_METRICS` measures · source system) with its column contract published on `Data Dictionary` so IT can point Power Query / Power Automate at it later; Dashboard formulas already reference the table by range and read `—` until rows land. This was requirement (1) from Scott.
- **Conventions inside the workbooks:** yellow = input cells; `✔` dropdown check-offs; scores validated 1–10 with the app's anchor text printed under each question; C.I.I. items roll up on their own row and flag red below 6; sheets protected without a password as a guardrail only.
- **Defect found + fixed during the build:** master-checklist items are stored in *authoring* order, not phase order (Week 1 appears in six separate runs), so the naive contiguous-run grouping produced 21 repeated phase headers. Now groups into a dict keyed by phase and emits in canonical `MC["phases"]` order — 7 clean blocks, matching how the app builds `MD_ITEMS_BY_PHASE`.
- **Verification (2026-08-16):** LibreOffice **cannot load xlsx in this environment** ("source file could not be loaded"; even a 3-cell probe times out), so the xlsx skill's `recalc.py` was unusable. Verified instead with the pure-Python `formulas` package (`pip install formulas`): **~15k cells across all four workbooks evaluate with zero error values**, and the seeded HVE workbook's computed roster statuses reproduce the app exactly (Okonkwo Overdue at day 180/Month 3, Brennan Due at day 56/Week 6, Hargrove 3.3 + Nguyen 3.5 as the two C.I.I.-risk providers). Builder sets `fullCalcOnLoad` so Excel computes on first open. **Two formula traps found this way, both fixed:** `COUNTIF(rng,"?*")` miscounts styled-but-empty cells (use `COUNTA` over typed-value columns); and a non-applicable track must read blank, not `0%` — the APC completion row is gated on roster credential so physicians don't show phantom unfinished APC work.
- **Resume recipe after `/clear`:** `cd src/apps/mentorship-tracker/excel-export`; read `README.md` there; `pip install openpyxl` then `node extract-data.mjs && python3 build_workbooks.py` regenerates into `out/`. Verify with `formulas` per the README before handing files over. Curriculum edits go in the TSX/JSON source, never in the generated `.xlsx`.
- **Open items:** (a) whether the Epic Quick Reference SPA also has to move into Office, and in what shape — not addressed by this work; (b) whether the four workbooks want a "view all regions" rollup for a regional CMO; (c) `pip install openpyxl` is required in a fresh environment (not added to `package.json` — the generator is deliberately standalone, so no shared-file coordination with Noah was needed).

**Culture Integration redesign — dedicated CII panel + month-9/APC cadence + amber convention — shipped 2026-05-30 (Scott, branch `claude/stoic-rubin-4QC5k`, commits `9ef3fac` + `629e9a0`).** The Culture Integration metric card now opens its own full view (`tab="culture"`) instead of dumping into the questionnaire. New file `src/apps/mentorship-tracker/CulturePanel.tsx`: (1) a leadership **verdict banner** (green/amber/red, with band label, trend arrow + "since last checkpoint" delta, and "N/M checkpoints answered"); (2) an SVG **trend chart** with area fill, dashed thriving-threshold line at 7.0, score-labeled dots, and dashed placeholder rings for not-yet-reached phases; (3) **reverse-chronological stacked phase cards** (Month 12 on top → Month 3 bottom; APCs continue to Month 24), each showing the 4 culture-question mini-bars and expanding on click to per-question text + scores. **Cadence change (data):** added `ci1–ci4` to QP `m9` so culture now fires at **m3/m6/m9/m12** for everyone; APCs get extended checkpoints at **m15/m18/m21/m24** via `CULTURE_PHASES_APC_EXT`. `cultureScore(qa, pid, apcFlag)` and the Score-Trends chart both phase-adapt; seed CII data added for p1 m9, p5 m9/m15/m18. **Color convention:** every score-driven pink (`#ec4899`) middle-band signal replaced with amber (`#eab308`) to match the productivity tiles' green/amber/red; culture *branding* moved pink→amber-800 (`#78350f`/`#92400e`) across Score Trends, roster table, dot-modal, card headers. **"Future-Scheduled"** replaces the em dash for not-yet-reached phases. **Two bug/structure fixes (commit `629e9a0`):** (a) the culture tile was a silent no-op — the JSX passed `apc={apc}` but `apc` was scoped inside the metric-card closure, so clicking threw a `ReferenceError` and aborted the render; now computes `isAPC(prov)` inline. (b) For APC (NP/PA) providers the "Mentor: Physician" tile was redundant — APCs run the same year-1 curriculum then continue into year-2 APC phases, so both collapsed into a **single "Mentor: APC" tile** spanning the full `MP` track (Week 0 → Month 24) via new `mentorFullPct()`; physicians keep "Mentor: Physician"; the separate "Year 2 · M15–M24" card + its grid column were removed. Verified headless (Onboarding Tracker → Primary Care → Dr. Rivera): physician shows Mentor: Physician + working culture panel (verdict "7.1/10 · Thriving ↑ +1.3 · 16/16"); APC (T. Singh) shows only Mentor: APC spanning W0→M24; zero page errors. `npm run build` clean. Same in-flight branch as productivity + data-export work; PR not yet opened.

**Productivity metrics on the Onboarding Tracker provider profile — shipped 2026-05-30 (Scott, branch `claude/stoic-rubin-4QC5k`).** Pipes Epic provider-productivity into the same view as onboarding tracking data: patients/day, RVUs/wk, in-basket time, time per note, Rx refill time — each a weekly trend vs. a practice benchmark. New file `src/apps/mentorship-tracker/ProductivityPanel.tsx` holds seed data + analysis + UI: 5 compact status tiles (big value, benchmark, plain-language status pill, progress gauge) → click a tile to expand into a full weekly trend chart with benchmark line, plain-language verdict banner, and 3 headline stats. Status color follows the locked green=good / amber=variable / red=wrong-direction convention; inconsistent providers get distinct "Variable" framing (zigzag line + "±swing" stat) instead of a false gain rate. Wired into `MentorshipTrackerApp.tsx` as a **director-view metric card** (the profile's clickable metric cards ARE the tab selectors): a "Productivity N/5 on track" card appears in the card row (gated on `hasProductivity`), clicking it sets `tab="prod"` which renders `<ProductivityPanel>` in place of the phase selector + checklist. Card-grid `colCount` made dynamic; `prodSummary()` export feeds the on-track count. **Scope note:** the metric-card tab system is director-only, so productivity shows in the director provider view (the executive audience); mentor view is unchanged. **Epic-pipe-ready shape:** the Epic integration populates `PROD_SERIES[providerId][metric]` (weekly arrays) + `PROD_METRICS[].goal` (benchmark) with no UI change — today both are seeded for the 9 demo providers. `tsc -b` + `npm run build` clean; verified in-app headless (tab → tiles → expand-to-chart, both "Climbing" and "Variable" states render). PR not yet opened — same in-flight branch as the data export/import work below. **Resume recipe:** nothing pending; future Retention-Risk + IDP features (still on the backlog) can read the same `PROD_SERIES` shape. **Label convention reaffirmed this session:** spell out "Medical Director" everywhere it's the role; keep "MD" only as the degree (parallel to DO / NP / PA).
...
**Lab QuickReply app — shipped to main 2026-05-30 via PR #15 (squash `f8ecf84`, admin override).** Sixth launcher app, emerald→teal gradient 🧪, sits to the right of Dashboard. Port of `~/incoming_noah/labmessenger.js` (Dr. Freiberg's lab-result portal-message wording aid: pick how the labs look → does the patient already have an appointment → if not, pick a return interval → composed message with greeting + sign-off + copy-to-MyChart; serious results trigger a phone-call guardrail instead of composing a message). Ported clean (smart quotes → ASCII; Tailwind → scoped CSS in `src/apps/lab-quickreply/lab-quickreply.css`; lucide → inline SVGs; `window.storage` → localStorage at `meridian-os.labQuickReply.provider.v1`; "CareMount" → "Optum NY/NJ"). Wiring per the BriefingShell/DashboardShell template (`launcherState.ts` union + `Launcher.tsx` 6th tile + `glass.css` gradient + `main.tsx` shell mount). Includes a follow-up fix for a CSS-specificity bug that hid the copy/edit button text + icon color on the dark-slate buttons (a scoped `.lab-quickreply button { color: inherit }` reset was outranking single-class `color: #fff` overrides; reset block deleted since `reset.css` provides the same globally). MERGEABLE/no conflicts; BLOCKED only on required review → admin-override squash like #10/#11/#13/#14. Production auto-deploy fired on `f8ecf84`. See Shipped milestone below.

**Onboarding Tracker meeting-log + label updates — shipped to main 2026-05-27 via PR #14 (squash `c7ca0d2`, admin override).** Scott's branch `claude/pensive-noether-gATBP` (1:1 meeting log per touchpoint phase + "All Providers"→"New Primary Care Providers" sidebar header + "Mentor Panels" promoted to a section heading; all in `MentorshipTrackerApp.tsx`). MERGEABLE/no conflicts; BLOCKED only on the required review, so admin-override squash like #10/#11/#13. Production auto-deploy fired and **succeeded** on `c7ca0d2`. See Shipped milestone below.

**⚠️ Cloudflare preview-branch auto-build is dropping subsequent pushes (open infra issue, 2026-05-27).** Symptom on `claude/pensive-noether-gATBP`: the **first** push built (`bbd568c`, Cloudflare check = success), but every push after it produced **no Cloudflare check at all** (`0bd4656`, and even an empty `1291947` "trigger Cloudflare preview build" commit). **Ruled out:** build quota (Noah: 38/100,000 used), branch-control + Git-connection config (Noah confirmed both healthy), and the known PR-timing gotcha (that suppresses only the *URL comment*; here the build never *ran*). **Diagnosis:** GitHub recorded the push (PushEvent 12:40:58Z), so the drop is the **Cloudflare Pages GitHub-App push webhook** (App-level — not inspectable/redeliverable by a repo admin; no repo-level webhook exists). **Still working:** manual dashboard redeploy, and the **production (`main`) auto-deploy** (PR #14's merge built fine) — so it's specific to preview-branch follow-up pushes. **Fix (Noah, dashboard):** disconnect+reconnect / re-authorize the Cloudflare Pages GitHub App (Settings → Builds & deployments → Git → manage), or uninstall+reinstall the App on the repo — re-establishes webhook delivery; won't disturb project config. **Stopgap for a preview without the trigger:** dashboard → Create deployment (builds branch HEAD), or `npm run build && npx wrangler pages deploy dist --project-name=meridian-os --branch=<branch>`. **Empty "trigger" commits do NOT help** (proven). Opening a fresh branch only resets the trap — its *first* push may build, its *second* will likely drop again until the App is reconnected.
...
### Onboarding Tracker — 1:1 meeting log + sidebar label/heading updates (2026-05-27, Scott, PR #14, squash `c7ca0d2`)

Squash-merged to main via PR #14 (branch `claude/pensive-noether-gATBP`, admin-override — BLOCKED on required review, no reviewer online; MERGEABLE with no conflicts). Three commits, all in `src/apps/mentorship-tracker/MentorshipTrackerApp.tsx` (Scott's territory; file stays `@ts-nocheck`, no schema/type changes):

- **1:1 Meeting Log per touchpoint phase** (`bbd568c`) — each mentor-checklist phase and provider-questionnaire phase gains a blue-band Meeting Log between the checklist/questionnaire and the quick-notes scratch pad. Structured per-phase 1:1 entry (date + free-text discussion + dynamic action-item list with per-item done checkboxes); collapsible cards newest-first with date / truncated preview / action-item completion badge. Persists to localStorage alongside checks/qa/notes; included in JSON + DOCX export (one dated section per entry, ☑/☐ action lines); cleared by reset flows + `doReset`.
- **Sidebar label + heading polish** (`0bd4656`) — "All Providers" → "New Primary Care Providers" director sidebar header; "Mentor Panels" promoted from muted uppercase caption to a bold 15px/800 section heading with a bottom-border rule.

**Merge:** admin-override squash; branch auto-deleted; production auto-deploy succeeded on `c7ca0d2`. **Process note:** this PR is where the Cloudflare preview-branch auto-build drop (see ⚠️ bullet in Current state) surfaced — Scott's later two commits never got preview builds; the empty `1291947` "trigger Cloudflare preview build" commit on the branch is an artifact of that (it produced no build either).
