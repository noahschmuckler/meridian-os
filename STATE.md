...
**ADHD module v2 (schema 1.3.0 / Simplified-Stratified Pass) — PR #1 open, head `9b0acd1` on `noah/adhd-module-v2-simplified-pass`.** PR: https://github.com/noahschmuckler/meridian-os/pull/1 (base = main). Cloudflare preview auto-deploys at `<sha-prefix>.meridian-os.pages.dev` per latest commit. Branch is now up-to-date with main (merged Scott's PR #2 OM-track work 2026-05-13; CLAUDE.md conflict resolved by relocating Scott's added bullet to Shipped milestones below). First module to adopt schema 1.3.0 two-tier FAQ shape (`first_layer_html` + `sub_questions[]`) plus per-entry `smartphrase_note` + `consult_decision_point` + module-level `smartphrases[]` registry. Other 6 modules untouched (still 1.0.0/1.0.1/1.2.0; back-compat via FAQ-bubble fallback path). Content sourced from `~/incoming_noah/ADHD-Stimulants-Inherited-v2.docx`; editorial conventions defined in `~/incoming_noah/meridian-module-simplification-standards.pdf` (Section 8 6-step checklist stamped in `verification/adhd.md`). 95-entry references superset preserved. CV monitoring split out as its own escalation + FAQ. **Future Scope C reference UI** (single filterable topic-reference primitive) bookmarked from `~/incoming_noah/meridian-adhd-faq-prototype.jsx`; deferred. **Resume recipe after `/clear`:** read `~/.claude/plans/mutable-stirring-conway.md` end-to-end; the plan is the implementation guide and is byte-stable with this commit's surfaces. **Next session likely tasks:** address PR review feedback after Scott reviews / Noah eyeballs preview deploy; then apply the same Simplification Pass to opiates + benzos using ADHD as the template.
