...
- **Resume recipe after `/clear`:** `cd src/apps/mentorship-tracker/excel-export`; read `README.md` there; `node extract-data.mjs && python3 build_workbooks.py` regenerates; **always** finish with `python3 /root/.claude/skills/synced/xlsx/scripts/recalc.py out/<file>.xlsx` (openpyxl writes formulas with no cached values, so an un-recalculated workbook opens with blank-looking computed cells). Curriculum edits go in the TSX/JSON source, never in the generated `.xlsx`.
