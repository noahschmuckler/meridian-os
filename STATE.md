...
**SmartPhrases (Track 4c) — implemented 2026-05-25 on THIS branch (PR #11).** Populated all **19 SmartPhrases** from `~/Downloads/smartphrase-implementation-spec.md` into the three CS modules' `smartphrases[]` registries (ADHD 6, benzos 6, opiates 7 — 3 confirmed/ship + 16 future), each with full HPI-compatible text + `decision_point` + `anchor`. The spec is the authoritative confirmation of the future-phrase names (resolves the earlier "confirm Claude-proposed IDs" follow-up); several were renamed to match (`.BENZO-COMBO-OPIATE`→`.BENZO-OPIATE-COMBO`, `.OPIATES-OUD-TRANSITION`→`.OPIATES-OUD-EVAL`, `.CSAGREE`→`.OPIATES-CSAGREE`, etc.). Used the **corrected** `.BENZO-NOINDICATION-BRIDGE` spelling (Noah's call; spec recommended normalizing). Built the spec data by **parsing the spec's fenced phrase-text blocks** (`/tmp/build_smartphrases.py`) — no transcription; the 4 non-CS modules stayed byte-identical. **Three surfaces (all delivered):** (1) new `smartphrase-selector` bubble (`src/bubbles/smartphrase-selector/`, purple) — lists a module's phrases grouped Ship-ready / Future, each expand→copy; (2) **expandable FAQ pills** — `smartphrase_note` pills now resolve their `.TRIGGER` against the registry and expand to the full text + Copy (`SmartPhraseNotePill` in `clinical-module-faq`); (3) **green-zone affordance** in the checklist — the continuation phrase is now a click-to-copy chip + an "All SmartPhrases →" button that spawns the selector (the primary module-mode entry; no auto-spawn, to avoid a 3rd auto-spawned bubble). Also a clinical-tools "SmartPhrases" card (gallery re-spawn path). Wiring: `src/types.ts` (union + `decision_point`/`anchor` on `ModuleSmartPhrase`), `src/bubbles/{index.ts,labels.ts}`, `src/styles/glass.css`, `src/shell/BspWorkspace.tsx`, `src/bubbles/clinical-tools/index.tsx`. **Verified:** `npm run build` clean; all 19 phrases carry text; every green-zone + FAQ-note trigger resolves to a registry id. **This completes the QOL roadmap — all of Tracks 1–4 (incl. 4a/4b/4c/4d) are now shipped.** Epic-side notes in the spec (Admin entry, pick-list `{braces}` conversion) are out of Meridian's scope (no Epic integration).

**Controlled-substances contract builder (Track 4d) — implemented 2026-05-25; combined onto THIS branch per Noah's request so the redrafted modules + the builder preview together (originally PR #12 off main; PR #12 to be closed).** New `contract-builder` bubble (purple `#6B4F9B`) + `src/data/seed/controlled-substances-contracts.json` clause library (v1.0.0 schema, 5 sections / 36 clauses, NY/NJ text variants, declarative `condition` objects). Provider sets substance class / state / situational flags → builder auto-suggests a risk tier (standard/enhanced/high-risk, Section-2 logic, override logged) → assembles matching clauses → outputs agreement text + `.CSAGREE-[CLASS]-[STATE]-[TIER]` trigger + flags summary + next-review date. Class-driven (not per-module). **Surfacing fix (the original "I don't see it"):** the clinical-tools Contracts card can't appear in module mode — the tools bubble is gallery-layout-only and the card is gated on an active module, so the two conditions are mutually exclusive. `ContractAutoSpawn` (app-level, `src/shell/ContractAutoSpawn.tsx`, mounted in `main.tsx`, mirrors `ConsultAutoSpawn`) now auto-spawns the builder on first entry to a CS module (adhd→stimulant / opiates→opioid / benzos→benzo), one-shot per module via `meridian-os.contractBuilderShown`. Files: `src/bubbles/contract-builder/index.tsx`, `src/shell/ContractAutoSpawn.tsx`, `src/data/seed/controlled-substances-contracts.json`, `src/types.ts` (Cs* interfaces + union), `src/bubbles/{index.ts,labels.ts}`, `src/styles/glass.css`, `src/shell/BspWorkspace.tsx`, `src/bubbles/clinical-tools/index.tsx`, `src/main.tsx`. Build helper `/tmp/build_cs_contracts.py`. **v1 deferrals:** DOCX/PDF export (copy-only); Section-5 per-class subsections; agreement-mention decorator. Bracketed tokens pass through as Epic fill-ins; only `[MEDICATION CLASS]` + `[VERSION STAMP]` substituted. **Open follow-ups for Noah:** (a) the boolean inputs can't express the schema's "significantly above" / "2+" magnitude wording for tier auto-suggest — mapped conservatively, override covers it; finer inputs in schema v1.1 if wanted. (b) Both consult + contract now auto-spawn on CS-module entry (2 extra bubbles) — flag if too crowded and I'll switch the contract one to a launch affordance. **Verified:** `npm run build` clean; tier/assembly logic simulated across 4 scenarios. **Surfacing verified by build + structural parity with the shipped ConsultAutoSpawn — confirm visually on the preview.**

**Opiates + Benzos modules v1.3.0 — Simplified/Stratified Pass (Noah, branch `noah/opiates-benzos-v13-simplification`, PR #11; now also carries the Track 4d contract builder above).** Both controlled-substance modules bumped 1.2.0 → 1.3.0 from Claude-chat-authored v2 DOCX files (`~/Downloads/Opiates-Inherited-v2.docx`, `~/Downloads/Benzos-Inherited-v2.docx`) — same workflow as ADHD's v2 DOCX. Implemented via surgical-splice scripts `/tmp/build_opiates_v13.py` + `/tmp/build_benzos_v13.py` (each replaces only its module object; other modules byte-identical). **Each module:** all 10 FAQ topics converted to two-tier `first_layer_html` + `sub_questions[]`; **a new 11th standalone topic** (`opiates-first-visit` = 11-element first-visit framework; `benzos-taper-primer` = rate/agent/sequencing/inpatient — each the §5 topic-split, `referenced_by: []`); footer → 2 sentences; module-level `smartphrases[]` registry (opiates 1 confirmed `.OPIATES-CONT` + 5 future; benzos 1 confirmed `.BENZO-CONT` + 5 future); per-FAQ `consult_decision_point`s wired to the existing consults (opiates: 5 → pain-medicine/addiction-medicine-opiate; benzos: 4 → addiction-medicine-benzo); references preserved verbatim at the evidence-confirmation superset (opiates 92, benzos 93 — all DOCX-cited refs were already present, 0 undeclared markers). Section 8 stamps prepended to `verification/opiates.md` + `verification/benzos.md`. **Verified:** `npm run build` clean, HTML tag-balance clean, every `faq_ref` resolves. **With this, all three controlled-substance modules (adhd/opiates/benzos) are at 1.3.0; lipid (1.1.0) is the only remaining simplification-pass candidate.** **SmartPhrase IDs now confirmed** by the Track 4c spec (entry above) — the registries were rebuilt to the spec's 19 phrases with full text. **Build scripts are self-contained** (content baked in) — re-runnable if needed. The Track 4d contract builder + Track 4c SmartPhrases are now on this same branch (entries above).
