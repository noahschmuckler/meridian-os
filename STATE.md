...
**Dashboard app — PR open 2026-05-22 (Noah, branch `noah/dashboard-app`).** New fifth launcher app, sits right of Briefing. Reformat of `~/Downloads/meridian-dashboard.jsx` (Crystal Run provider dashboard accordion POC) into a 6-tile Briefing-visual-language grid: 5 director-facing source sections (HEDIS / AWV / wRVU / Epic Goals / VBC+HCC) each rendered as a big-number colored tile, plus a 6th teal "Patients Tomorrow" tile. Clicking any tile expands it to fill the full content area; clicking the patient tile shows a 5-row roster (Jane Doe / Maria Rodriguez / Robert Johnson / Tyler Williams / Patricia Kim, copied from `meridian-onboarding/index-rendered.html` `demoPatients`) with major opportunity chips; clicking a patient row opens a full-bleed detail view (masthead + coral Hard Stops banner + Problem List / Vitals / Labs / Medications grid + Opportunities cards with smartphrase/code styling). Files: `src/apps/dashboard/{DashboardApp.tsx,dashboard.css}` (scoped under `.dashboard-app` per the Briefing collision-avoidance pattern) + `src/shell/DashboardShell.tsx` + `src/data/seed/dashboard-cohort.json` (5 demo patients). Six-touchpoint launcher add: type extension, route branch, button, icon-variant gradient (plum `#4a1d6e` → amber `#d4a017`), shell, app. Build clean (`npm run build` + `tsc -b` clean). Visual smoke-test passed (all 5 section drill-ins, AWV unscheduled drawer, both patient detail variants, portrait toggle) via headless Chrome — screenshots in `/tmp/dashboard-shots/`. **Resume recipe after `/clear`:** check PR status (`gh pr list --head noah/dashboard-app`); if visual issues on preview, key files are DashboardApp.tsx + dashboard.css pair; on merge, bump this entry to Shipped milestones below.

**Briefing app — PR #8 shipped 2026-05-22 (Noah).** Squash commit `4925b48` on main. New fourth launcher app — port of `~/Downloads/meridian-briefing.html` (Crystal Run Provider Briefing POC) into Preact/TSX. Files: `src/apps/briefing/BriefingApp.tsx` + `src/apps/briefing/briefing.css` (all selectors scoped under `.briefing-app` so the briefing's `.bubble` rule doesn't collide with meridian-os's first-class bubble primitives) + `src/shell/BriefingShell.tsx`. Six-touchpoint launcher add: type extension, route branch, button, icon-variant gradient (navy `#1a2744` → teal `#00788a`), shell, app. Briefing app then served as the visual-language template for the Dashboard app (see entry above) — DM Sans + DM Serif Display, parchment background, scoped color-tinted "brief cards", masthead + footer + orientation toggle + print, single-panel-fills-on-click drill-in. The two apps share the `--crh-*` palette token names verbatim.
