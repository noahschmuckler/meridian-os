...
**Mentorship Tracker — Full Mentor Curriculum + APC Track (Scott, branch `claude/serene-euler-vF8et`, HEAD `f8885c2`, 2026-05-22).** Replaced the 14-phase stub `MP` with Noah's complete Mentor Curriculum: 17-phase Physician track (Week 0 Pre-Start → Month 12 Year One Completion, 128 items) + 4-phase APC track (Month 15–24, 29 items) for a total of 157 items. Key changes: (a) `MP_PHYS` + `MP_APC` + `MP = [...MP_PHYS, ...MP_APC]`; (b) `isAPC()` helper checks role NP/PA; (c) `mentorPhysPct()` + `mentorApcPct()` split helpers; (d) "Mentor Curriculum" metric tile split into "Mentor: Physician" and "Mentor: APC" (APC tile only shown when viewing an NP/PA provider); (e) both tracks accessible in "View All Curriculum" modal as separate color-coded sections; (f) Phase IDs updated: w4→m1, w8→m2, q3→m9, q4→m12; (g) Dr. Martinez (NP) added as APC demo provider at Month 18. **Playwright stress test PASSED (2026-05-22):** all 5 providers render; Dr. Martinez shows 6-tile metric grid with "Mentor: Physician" (100%) + "Mentor: APC" (50%); APC phases M15/M18/M21/M24 navigate correctly; Week 0 at 100% (all checked); Month 12 renders with year-in-review item + APC continuation note; Dr. Johnson (MD) shows no APC tile and no Month 15 button; View All Curriculum modal has all 4 tracks (MD / Mentor Physician teal / Mentor APC purple / Office Manager); mentor-role users correctly cannot see View All Curriculum; comparison grid shows "Mentor: Physician" column. One expected console error: `net::ERR_CERT_AUTHORITY_INVALID` for `/api/chat` (Cloudflare Workers not available in local dev). **Not yet merged to main** — PR pending user request. **Resume recipe after `/clear`:** branch is pushed; file is `src/apps/mentorship-tracker/MentorshipTrackerApp.tsx`; open PR if user requests it.
