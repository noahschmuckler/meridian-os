...
- **Latest commit:** `f8a729d` (Floating top-left iOS-glass back chevrons: `‹ Mondrian` returns from any Mondrian workspace to the selector grid; `‹ modules` returns from clinical-modules module mode to gallery. Stack vertically when both visible — outer scope on top at 16px, inner scope below at 60px. Replaces the easy-to-miss inline `‹ modules` button buried in the checklist chrome — alpha tester had been reaching for browser back. Predecessor `7eee29a` rewrote the ADHD module to v1.1.0 with the 95-reference evidence pass.)
- **ADHD module evidence verification — Phase 1 (OE pass) + Phase 2 (JSON rewrite) complete (2026-05-03).** Phase 1 resolved all 74 tracker rows via 8 OE bundles + 2 deep-dive follow-ups + 1 cross-module regulatory pass (`~/GitHub_Repos/meridian-server/modules/adhd-verification.md`; cross-module regulatory snippets at `verification/controlled-substances-ny-nj.md`). Phase 2 applied those rows to `src/data/seed/clinical-modules.json` — markers render as raw `[ref:X]` text today. **Phase 3 (the references-rendering UI pass) is the immediate next work item** — see "Pending plan" below.
...
- **Phase 3 wiring (the next work item)** — `expandRefMarkers(html, module)` helper called from `src/bubbles/clinical-module-faq/index.tsx` and `src/shell/PrintView.tsx` to render markers as superscripts; strip the `\[ref:[a-z0-9-]+\]` regex from `htmlToPlain` in `src/lib/generateDocx.ts` and `src/lib/generatePptx.ts` so markers don't leak into Word/PowerPoint plain text; add `.ref-marker` / `.faq-references` CSS to `src/styles/glass.css`. Round-trip test: export DOCX → re-import, confirm `[ref:X]` markers survive.
...
### Immediate next: ADHD module v1.1.0 — Phase 3 (references-rendering UI)

Phase 2 has shipped the JSON. Markers render as raw `[ref:X]` text inside `answer_html` today. Phase 3 is the **code-only** pass that turns markers into rendered superscripts in the bubble UI + print view, strips them from DOCX/PPTX exports, and verifies round-trip via export → re-import.

**Phase 3 to-do, in one paragraph:**

1. Add `expandRefMarkers(html, module)` helper that walks an `answer_html` string, replaces `[ref:X]` matches with a superscript anchor (`<sup class="ref-marker"><a href="#ref-X" data-ref="X">[X]</a></sup>` or similar), and returns the augmented HTML. Helper lives in `src/lib/refMarkers.ts` (new file).
2. Call it from `src/bubbles/clinical-module-faq/index.tsx:124` (FAQ answer renderer) and `src/shell/PrintView.tsx:79` (print FAQ section). The print view should also append a `.faq-references` block at the end of each FAQ (or at the end of the module) listing every cited ref with its full citation + URL.
3. Strip the `\[ref:[a-z0-9-]+\]` regex from `htmlToPlain` in `src/lib/generateDocx.ts:22-34` and the matching helper in `src/lib/generatePptx.ts` so markers don't leak into Word/PowerPoint plain text. Verify the DOCX/PPTX exports look clean.
4. Add `.ref-marker` (small, superscript, accent-colored) and `.faq-references` (compact list, smaller font) styles to `src/styles/glass.css`. Apply at print as well (`@media print`).
5. Round-trip test: export the rewritten ADHD module to DOCX from the green checklist chrome, re-import via the clinical-tools "Import .docx" button, confirm `[ref:X]` markers survive (the parser shouldn't strip them on the way back in).
6. Optional polish: hover/click on a `.ref-marker` could pop a tooltip with the citation. Defer if scope creeps.

**Plan details:** `~/.claude/plans/i-have-unfortunately-been-fluffy-alpaca.md`.

**Phase 4 (after Phase 3):** `./deploy.sh` — confirms the rewritten module on https://meridian-os.pages.dev.
