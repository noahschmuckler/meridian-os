...
- **SmartPhrases (Track 4c): registry data + three surfaces, no auto-spawn.** Module-level `smartphrases[]` (id / status confirmed|future / text / decision_point / anchor) is the registry; the green-zone `smartphrase` string and each FAQ `smartphrase_note` reference a phrase by its `.TRIGGER` token. Three surfaces: (1) the `smartphrase-selector` bubble (full list, expand→copy); (2) FAQ `smartphrase_note` pills that resolve their `.TRIGGER` against the registry and expand to full text + Copy (plain note if unresolved); (3) the checklist green-zone — click-to-copy continuation chip + "All SmartPhrases →" launch (the primary module-mode entry). **No auto-spawn** — consult + contract already auto-spawn on CS-module entry; a third would crowd the module view, so the selector is launch-only. Phrase text = HPI-compatible prose; `[bracketed]` tokens are Epic fill-ins, passed through verbatim. Spec source: `~/Downloads/smartphrase-implementation-spec.md`; Epic-side Admin entry / `{pick-list}` conversion is out of Meridian's scope.
