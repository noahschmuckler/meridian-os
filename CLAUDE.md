...
- **Latest commit:** `__BENZO_PHASE2_COMMIT__` (Benzo module v1.1.0 — Phase 2 JSON rewrite + Phase 4 deploy. `benzos` entry in `src/data/seed/clinical-modules.json` bumped 1.0.0 → 1.1.0; 93-entry top-level `references[]`; 223 inline `[ref:X]` markers across landing_intro (7) + green_zone.narrative_html (6) + context_strip.text (4) + 10 FAQs (206); 88 unique refs cited; 0 undeclared, 5 unused-as-adjacency. Highest-impact corrections: temazepam factual fix (10 mg → 20 mg ≈ diazepam 10 mg, was wrong by factor of 2 since v1.0.0); 40 mg/day threshold replaced by ASAM tier framework (low ≤10 / mod 10–15 / high >15); "always taper benzo first" disproved via Wang JCM 2023 + CDC 2016 + ASAM shared-decision-making; cognitive-reversibility reframe (partial/domain-specific per Bundle 8 DD1 — module text now explicitly does NOT promise improvement to MCI patients/families); recency-dependent overdose risk per Alobaidi 2021 replaces "risk increases with duration"; "black box" → "boxed warning" + OUD-OAT carve-out; Beers framing softened to "general rule with exceptions". Phase 3 was a no-op — opiates Phase 3 had already generalized `getModuleRefNumberer` + render sites to walk landing_intro + green_zone + context_strip + faqs + footer_note. Benzo numbering follows the same walk-order pattern. Predecessor `915df5d` shipped Opiates Phase 3.)
...
With ADHD + opiates + benzos all done end-to-end, the open workstreams are: **(a)** the Clinical Modules wow-factor wiring described below; **(b)** rewrite the four remaining v1.0.0 modules (lipid, ckd, anemia, abd-pain) the same way. Pick whichever the user prioritizes.
