...
- **Latest commit:** `2e9e48c` (Opiates module v1.1.0: Phase 2 JSON rewrite — `opiates` entry bumped to schema_version 1.1.0; 92-entry references[] array; 414 inline `[ref:X]` markers across 10 FAQs; six highest-priority corrections from the verification tracker landed (tramadol MME factor 0.1→0.2, cash-pay PDMP framing, opiate-benzo CDC Cat B language, cocaine UDS reframe, CSA "common elements" relabel). Predecessor `f13448c` shipped ADHD Phase 3: `[ref:X]` markers render as numbered `<sup>` superscripts in the FAQ bubble + print view; per-FAQ + module-level cited refs lists with URL anchors; DOCX round-trip preserves structured references; PPTX FAQ slides paginate at sentence boundaries with `(cont.)` continuation slides; print view's `html/body/#app` overflow lock reset under @media print so multi-page modules actually paginate.)
...
- **Opiates module evidence verification — Phase 1 (OE pass) + Phase 2 (JSON rewrite) complete (2026-05-03).** Phase 1 fully populated at `verification/opiates.md` (108 of 108 tracker rows resolved across 8 OE bundles + meta-pass + cross-module regulatory hand-off + NY+NJ dual-state scope decision). Phase 2 applied to `src/data/seed/clinical-modules.json`: `opiates` entry bumped 1.0.0 → 1.1.0; 92-entry `references[]`; 414 inline `[ref:X]` markers across 10 FAQs (90 unique refs cited; 0 undeclared, 2 unused). Highest-priority corrections from the tracker (tramadol MME factor, cash-pay PDMP framing, opiate-benzo "particular caution," cocaine UDS, CSA common-elements) all landed. Phase 3 is automatic — markers render via the same helper ADHD uses.
...
- ~~Opiates module Phase 1 (OE pass) + Phase 2 (JSON rewrite)~~ — 108 of 108 tracker rows resolved at `verification/opiates.md`; `opiates` entry bumped 1.0.0 → 1.1.0; 92 entries in `references[]`; 414 `[ref:X]` markers across 10 FAQs (90 unique cited; 0 undeclared, 2 unused). Six highest-priority corrections from the tracker landed (tramadol MME factor, cash-pay PDMP framing, opiate-benzo Cat B language, cocaine UDS, CSA common-elements). Phase 3 wiring is automatic — same `expandRefMarkers` helper renders the opiates markers.
