...
- **Latest commit:** `915df5d` (Opiates Phase 3 + helper extension: `getModuleRefNumberer` now walks landing_intro + green_zone.narrative_html in addition to context_strip + faqs + footer_note, in **print-page reading order** so superscripts read 1,2,3 top-to-bottom on the printed page. PrintView now expands `[ref:X]` markers in landing_intro / green_zone / context_strip / footer_note (previously only answer_html). Clinical-module-checklist bubble strips markers from landing_intro / context_strip / footer_note since it has no references section — mirrors DOCX/PPTX export behavior. Surfaced because the opiates rewrite uses landing_intro + green_zone for citations; ADHD's rewrite happened to confine markers to faqs + context_strip so the gap was invisible there. Predecessor `2e9e48c` shipped Opiates Phase 2 JSON rewrite. ADHD numbering shifts incidentally: context_strip refs now claim numbers 1..5 ahead of FAQ refs.)
...
With ADHD + opiates both done end-to-end, the open workstreams are: **(a)** Phase 1+2 the **benzo module** (research already staged at `verification/benzos.md` + `verification/benzo1-4.md` + `verification/benzo5-8.md`); **(b)** the Clinical Modules wow-factor wiring described below; **(c)** rewrite the four remaining v1.0.0 modules (lipid, ckd, anemia, abd-pain) the same way. Pick whichever the user prioritizes.
...
src/
  ...
  data/
    ...
    seed/clinical-modules.json  All 7 bundled modules (lipids, ckd, adhd, anemia, abd-pain, opiates, benzos). 6 of 7 remain byte-identical to vanilla's modules/*.json; the `adhd` entry is at schema_version 1.1.0 with a top-level `references[]` array and inline `[ref:X]` markers across all 8 FAQs (Phase 2 rewrite, 2026-05-03). meridian-os is the single source of truth for the rewritten adhd module — the frozen sibling at `~/GitHub_Repos/meridian-server/modules/adhd.json` is intentionally not updated.
