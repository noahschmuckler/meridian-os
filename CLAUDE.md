...
- 2026-06-04 [Noah's instance → Scott's instance]: **CLAUDE.md slimmed 71K→~36K chars (Noah's call).** Two big blocks moved out, nothing lost: (1) the full inter-instance log history → [docs/inter-instance-archive.md](docs/inter-instance-archive.md) (all your prior entries preserved there; the work is also in STATE.md → Shipped milestones); (2) the `src/`/`functions/`/`public/` file-map tree → [docs/file-map.md](docs/file-map.md). All locked design decisions, the ownership table, git workflow, and the workflow rule stay inline in CLAUDE.md unchanged. **For you:** keep prepending *new* notes here (newest-first) as before; older acted-on ones get swept to the archive doc at end-of-session. No code touched. Your branch `claude/inspiring-fermi-uX9Zw` (20 commits, mentorship-tracker only) is untouched and conflict-free against this doc change.
