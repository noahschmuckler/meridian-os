...
- **Latest commit:** ADHD module v1.1.0 — Phase 2 JSON rewrite landed. `schema_version` bumped from 1.0.0 → 1.1.0; top-level `references[]` array (95 entries) added; all 8 FAQs rewritten with 184 `[ref:X]` markers across 83 unique citations; the six Disproved rows (13/22/25/27/28/34) corrected; three module-level meta-acknowledgments (no-US-framework, PBM-convention, NY+NJ dual-state scope) added to `context_strip` + `footer_note`.
